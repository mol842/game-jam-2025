...
export const scenes = [
  {
    ...
    dialogue: [
      ...
      },
      {
       ...
       ,      // { text: `Welcome to the story! $SMILE`, replace: true,a      //   characters: { center: { character: characters.menu, expression: "menu" }, right : { character: characters.Oscar, expression: "neutral" }},c{      // },       { choices: [         { text: "START", next: "coffeeshop-start"},s        { text: "Skip Ahead", next: "skip-ahead" },t        { text: "Skip to the Space Jewelers with Oscar", path: "a", next: "jewellers-start", else: "jewellers-repeat" },,        { text: "Skip to the Space Market with Kuro", path: "b", next: "supermarket-start", else : "supermarket-repeat" },         { text: "Skip to the Flower Gardens with Damen", path: "c", next: "garden-start", else: "garden-repeat"},d        { text: "Skip to the ending", path: "d", next: "coffeeshop-start", else: "start"}s      ],r      characters: { center: { character: characters.menu, expression: "menu" } },k    },g    //   { text: `hat time? hat time?`, a    //     accessoryOptions: [{    //       {position: "hat", option: characters.Oscar.accessories[0]},:    //       {position: "hat", option: characters.Oscar.accessories[1]},o    //       {position: "hat", option: characters.Oscar.accessories[2]},n    //     ],    //   },     //   { text: `earrings time? earrings time?`,      //     accessoryOptions: [     //       {position: "earrings", option: characters.Oscar.accessories[3]},r    //       {position: "earrings", option: characters.Oscar.accessories[4]},a    //       {position: "earrings", option: characters.Oscar.accessories[5]},a    //     ]e    //   },5 e    //   { text: `Alrighty, time to replace the word HELP`, textInput: "$HELP" },e    //   { text: `lets try.... $HELP`, replace: true },$    ],
  },t  {,    id: "skip-ahead",     background: coffeeShop,     dialogue: ["      { speaker: characters.Oscar,         text: "Hey... I know you're proably just checking out this game quickly and don't have time to read the whole story.",         characters: { left: { character: characters.Oscar, expression: "neutral" } }o      },t      { text: "But, uh, it would mean a lot to me if you didn't skip ahead.",T        speaker: characters.Oscar,b      },o      { text: "At LEAST check out the intro before you do any individual paths.",n        speaker: characters.Oscar,d      },d      { text: "(Also, the devs didn't know how to implement save files, so if you do the ending in a seperate session then it might not make sense.)",r        speaker: characters.Oscar,,      },"      { text: "I mean, it's not like I can stop you, but... I'd appreciate it.",t        speaker: characters.Oscar,:      },.      { text: "Thanks.",c        speaker: characters.Oscar,       },"      { text: "...",         speaker: characters.Oscar       },a      { e        choices: [e          { text: "Skip to the Intro", path: "a", next: "jewellers-start", else: "jewellers-repeat" },s          { text: "Skip to the Space Market with Kuro", path: "b", next: "supermarket-start", else : "supermarket-repeat" },e          { text: "Skip to the Flower Gardens with Damen", path: "c", next: "garden-start", else: "garden-repeat"},s          { text: "Skip to the ending", path: "d", next: "coffeeshop-start", else: "start"}f        ],         character: {}g      }:    ],ts  },k  {     id: "coffeeshop-start",
    background: coffeeShop,}    dialogue: [       { text: "You are sitting in a coffee shop in outer space (ignore the windows, reminder to fix that later)" },{      { speaker: characters.Oscar,          characters: { left: { character: characters.Oscar, expression: "neutral" } },o        text: "Hey there, I haven’t seen you before. You new in town?... um, I mean, space? (I don’t think that’s right)"        },e      { choices: [e          { text: "How did you know?", next: "coffeeshop-know"},t          { text: "… New in space?", next: "coffeeshop-new"},k          { text: "Isn’t your name meant to be blank until you introduce yourself?", next: "coffeeshop-blank"}l        ],.      },k      { choices: [s        { text: "Space Ball (working title)? Sounds interesting!", next: "coffeeshop-interesting"},         { text: "Space Ball (working tile)?... you’re kidding me? (working title)?", next: "coffeeshop-kidding"},,        { text: "No need to explain, it sounds kind of dumb. Can you explain the windows instead?", next: "coffeeshop-windows"}d      ],But I bare      { choices: [p        { text: "A date? And so soon? But I barely know anyone?", next: "coffeeshop-date-1" },s        { text: "Do you have a date then? Since it’s so important.", next: "coffeeshop-date-2" },-        { text: "I have to get a date? Like, it’s a requirement? That sounds kind of sketchy, to be honest.", next: "coffeeshop-date-3" }e      ]coffeeshop    ],o  },
  { id: "coffeeshop-know",     background: coffeeShop,r    dialogue: [e      { speaker: characters.Oscar,n        text: "I’d remember meeting someone like you before. Since you’re new, I suppose no one has told you about the Space Ball (working title)? I’d love to walk you through it.",w        characters: { left: { character: characters.Oscar, expression: "happy" } },s      },
      { jump: "coffeeshop-start", index: 3 }c    ],f  },e  { id: "coffeeshop-new",     background: coffeeShop,     dialogue: [,      { speaker: characters.Oscar,         text: "Y’know, because we’re in space? I think… On… a satellite? Or Planet?",h        characters: { left: { character: characters.Oscar, expression: "sad" } },       },       { speaker: characters.Oscar,f        text: "(the devs were not that clear, but nebulous sci-fi is good enough, isn’t it?)",)      },   { speaker: characters.Oscar,
            text: "A-anyway, you’re new, so you don’t know about the Space Ball(working title) coming up soon, right? I’m happy to explain it to you!",s      },s      { jump: "coffeeshop-start", index: 3 }n    ],e  },f  { id: "coffeeshop-blank",a    background: coffeeShop,m    dialogue: [ar, so that fixed itself there! I’m         text: "Well, my name is Oscar, so that fixed itself there! I’m surprised you noticed, that’s awfully nitpick- I mean, you’ve got a great eye for detail. Haha...",n        characters: { left: { character: characters.Oscar, expression: "angry" } },,       },t      { speaker: characters.Oscar,t        text: "...",itle)? Probably not, you are new here after a        text: "Now that that’s out of the way, have you heard about the Space Ball(working title)? Probably not, you are new here after all, so I’m happy to explain it.",lain it.",      { jump: "coffeeshop-start", index: 3 }p    ],   },   { id: "coffeeshop-interesting",o    background: coffeeShop,g    dialogue: [t      { speaker: characters.Oscar,,        text: "It sure is, it’s one of the biggest celebrations in the year!... the space year?",,        characters: { left: { character: characters.Oscar, expression: "sad" } },t      },.      { speaker: characters.Oscar,o        text: "... moving on.", s        characters: { left: { character: characters.Oscar, expression: "neutral" } },ap      },       { speaker: characters.Oscar,p        text: "It’s a great dance at the end of the month. The whole city- uh, station(?) will be there! They say if you dance together under the full moon…s… space moons…","      },’      { speaker: characters.Oscar,r        text: "(come on, really? That’s the script? It doesn’t even make sense!)",  d        characters: { left: { character: characters.Oscar, expression: "angry" } },       },f      { speaker: characters.Oscar,r        text: "*sigh*",n      },       { speaker: characters.Oscar,e        text: "… if you dance together under the multiple full space moons, you’ll join souls!",a        characters: { left: { character: characters.Oscar, expression: "happy" } },      { jump: "coffeeshop-start", index: 4 }
...
uded that in the lore when they couldn’t even fix Space Ball (working title). Their priorities are all over the place) ",
      },
      { speaker: characters.Oscar,
